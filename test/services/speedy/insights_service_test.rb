...
class Speedy::InsightsServiceTest < ActiveSupport::TestCase
  include SpeedyTestHelper

  setup do
    access_token = SecureRandom.base58(16)
    @service = Speedy::InsightsService.new(access_token)
  ...
  #
  test '#query_aggregated_accounts_insights - will handle error from Speedy (on schema download)' do
    stub_speedy_schema_not_running

    assert_raises(Speedy::InsightsService::ApiException) do
      @service.query_aggregated_accounts_insights(@merchant_guid, @account_guids)
    end
  end

  test '#query_aggregated_accounts_insights - will handle timeout error from Speedy (on schema download)' do
    stub_speedy_schema_timeout
  ...
  test '#query_aggregated_accounts_insights - will handle error from Speedy' do
    stub_speedy_query_not_running
  ...
  test '#query_aggregated_accounts_insights - will handle timeout error from Speedy' do
    stub_speedy_query_timeout
  ...
  test '#query_aggregated_accounts_insights - recover from Speedy not running' do
    stub = stub_speedy_query_not_running
  ...
  test '#query_aggregated_accounts_insights - handles query error' do
    ...
    assert_raises Speedy::InsightsService::QueryError do
      @service.query_aggregated_accounts_insights(@merchant_guid, @account_guids)
    end
  end
  ...
  test '#query_aggregated_accounts_insights - raises an error on invalid aggregation (String)' do
    stub_speedy_schema
    stub_speedy_aggregated_accounts_insights_query_error
    assert_raises Speedy::InsightsService::AggregationError do
  ...
  test '#query_aggregated_accounts_insights - passes account guids and aggregation level to query' do
    ...
    stub_speedy_schema
    stub_speedy_aggregated_accounts_insights_query(account_guids: account_guids)

    @service.query_aggregated_accounts_insights(merchant_guid, account_guids, aggregation: aggregation)

    assert_requested(:post, speedy_graphql_url) do |request|
  ...
  test '#query_aggregated_accounts_insights - passes the correct access token' do
    ...
    assert_requested(:post, speedy_graphql_url, times: 2) do |request|
  ...
  test '#query_aggregated_accounts_insights - will return just the insights data' do
    ...
    stub_speedy_schema
    stub_speedy_aggregated_accounts_insights_query(account_guids: @account_guids, last_transaction_date: last_transaction_date)
  ...
  test '#query_aggregated_accounts_insights - returns empty balance + projection when nil is received from Speedy' do
    stub_speedy_schema
    stub_speedy_aggregated_accounts_insights_query_empty
    result = @service.query_aggregated_accounts_insights(@merchant_guid, @account_guids)
  ...
  test '#query_aggregated_accounts_insights - will return nil when no merchant is returned from the DWH' do
    Speedy::InsightsService.any_instance.stubs(:send_request).returns(OpenStruct.new(data: nil))
  ...
  test '#query_aggregated_accounts_insights - will return nil when no data is returned from the DWH' do
    Speedy::InsightsService.any_instance.stubs(:send_request).returns(nil)
